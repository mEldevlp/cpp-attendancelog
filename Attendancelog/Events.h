...
<<<<<<< HEAD
	...
	catch (...)
	{
		...
		MessageBox::Show("Количество человек должно быть целом числом!", "Ошибка",
			MessageBoxButtons::OK,
			MessageBoxIcon::Error);
	}	
}
...
void TableForm::button2_Click(System::Object^ sender, System::EventArgs^ e)
{
	if (textBox1->Text != "" || textBox2->Text != "")
	{
		auto result = MessageBox::Show("Вы ввели данные, уверены что хотите отменить создание?", "Сообщение",
			MessageBoxButtons::YesNo,
			MessageBoxIcon::Question);
...
void Attendancelog::TableForm::TableForm_FormClosing(System::Object^ sender, System::Windows::Forms::FormClosingEventArgs^ e)
{
	...
	{
		//System::Windows::Forms::DialogResult result;
		auto result = MessageBox::Show("Вы ввели данные, уверены что хотите отменить создание?", "Сообщение", MessageBoxButtons::YesNo, MessageBoxIcon::Question);
		if (result == System::Windows::Forms::DialogResult::Yes)
			TableForm::~TableForm();
		else
			e->Cancel = true;
